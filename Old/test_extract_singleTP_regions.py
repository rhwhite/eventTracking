...
if minimalvars == 1:
        f4vars = np.array(['xmaxtravel','gridboxspan','totalprecip','timespan','tstart','tmean'])
else:
...
def createfile(filenameM, filenameT, nlatsregin, f4varsin, f8varsin):

        filenameM.createDimension('events', None)
        filenameM.createDimension('latRegions',nlatsregin)
        filenameT.createDimension('events', None)
        filenameT.createDimension('latRegions',nlatsregin)

        filevarsM = []
        filevarsT = []
        for ivar in range(0,len(f4varsin)):
                filevarsM.append(filenameM.createVariable(f4varsin[ivar],'f4',('events'),fill_value=-9999))
                filevarsT.append(filenameT.createVariable(f4varsin[ivar],'f4',('events'),fill_value=-9999))

        for ivar in range(0,len(f8varsin)):
                filevarsM.append(filenameM.createVariable(f8varsin[ivar],'f8',('events'),fill_value=-9999))
                filevarsT.append(filenameT.createVariable(f8varsin[ivar],'f8',('events'),fill_value=-9999))

        return (filevarsM,filevarsT)


def returnbasin(xicenterin,yicenterin,dobasinrout):
	Basidx = np.nan	#default value
...
ibound = 0
FileO = 'test_' + fileOadd + 'Precip_Sizes_' + str(int(tbound[ibound])) + '-' + str(int(tbound[ibound+1])) + 'day_' + Data  + str(Fstartyr) + '-' + str(Fendyr) + '_' + Version + '.nc'

ncfileT = Dataset(DirO + 'AllTr' + FileO, 'w')
ncfileM = Dataset(DirO + 'AllMid' + FileO, 'w')
OfilevarsAll = (createfile(ncfileM,ncfileT,nlatregs,f4vars,f8vars))
...
if dobasin == 1:
	...
	filevarsPa = (createfile(ncfileM,ncfileT,nlatregs,f4vars,f8vars))
	ncfiles.append(ncfileT)
	ncfiles.append(ncfileM)

	ncfileT = Dataset(DirO + 'AtlanticTr' + FileO, 'w')
	ncfileM = Dataset(DirO + 'AtlanticMid' + FileO, 'w')
	filevarsAt = (createfile(ncfileM,ncfileT,nlatregs,f4vars,f8vars))
	ncfiles.append(ncfileT)
	ncfiles.append(ncfileM)

	ncfileT = Dataset(DirO + 'IndianTr' + FileO, 'w')
	ncfileM = Dataset(DirO + 'IndianMid' + FileO, 'w')
	filevarsIn = (createfile(ncfileM,ncfileT,nlatregs,f4vars,f8vars))
	ncfiles.append(ncfileT)
	ncfiles.append(ncfileM)
...
filevarsAll = np.zeros([2,len(f4vars)+1,nevents],np.float)

#Extract events of startlen-endlen days
for ievent in range(0,nevents):	#nevent
	# Get time index
	if timespan[ievent] < tboundper[1]:
		
		basindex, Tidx = returnbasin(xcentermean[ievent],ycentermean[ievent],dobasin)
...
print filevarsAll[0,0,writeidxAll[0]]
print filevarsAll.shape
print OfilevarsAll[0]
