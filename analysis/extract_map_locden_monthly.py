"""
This file extracts and converts to map format local density (i.e. number of
events crossing each grid point), and local fractional density (i.e. local
density but scaled by the total number of grid-points each event is spread
over, so the spatial sum equals the total number of events.
...
if splittype == 'day' and unit == 'day':
    calctbound1 = np.array(args.tbound1) * 24.0    # multiple by 24 to get hours, not days
    calctbound2 = np.array(args.tbound2) * 24.0
else:
    calctbound1 = args.tbound1
    calctbound2 = args.tbound2


diradd = getdirectory(splittype)

nbounds = len(calctbound1)
...
for iday in range(0,nbounds):
    ...
    for ievent in range(0,nevents):
        ...
        if evGridboxes[ievent] > minGB: 
            ...
            if splittype == 'day':
                ...
                if evduration > calctbound1[iday] and evduration <= calctbound2[iday]:
    ...
    daout = xr.DataArray(locDensity,
                            ...
                            attrs=[('description','local density of events'),
                                    ('units','event passes/gridbox/month')])
    ...
    daout2 = xr.DataArray(locFracDensity,
                            ...
                            attrs=[('description','local fractional event density'
                                    ...
                                    ('units','event presence/gridbox/month')])
    ...
    FileO = ('LocFracDensity_' + str(startyr) + '-' + str(endyr) + '_' + Version +
                   '_' + str(args.tbound1[iday]) + '-' + str(args.tbound2[iday]) + unit + '.nc')
