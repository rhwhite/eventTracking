...
for ibound in range(0,nbounds):
    ...
    for ivar in allvars:
        writeFiles[ibound].createVariable(ivar, 'f8', ('time','lat', 'lon'), fill_value=-9999)
    ...
    for ivar in extravars:
        if ivar == 'time':
            writeFiles[ibound].createVariable(ivar, 'f8', ('time'), fill_value=-9999)
            setattr(writeFiles[ibound][ivar], 'units', 'days since ' + str(startyr) + '-01-01')
            for iyear in range(0,nyears):
                for imonth in range(0,nmonths):
                    itime = (iyear*12) + imonth
                    writeFiles[ibound][ivar][itime] = dtime
